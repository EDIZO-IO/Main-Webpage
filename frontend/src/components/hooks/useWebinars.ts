...
export const useWebinars = () => {
  ...
  const fetchWebinars = async (): Promise<Webinar[]> => {
    ...
    try {
      ...
      const parsedWebinars: Webinar[] = data.values.map((row: string[], index: number) => {
  ...
  const loadWebinars = async (revalidate: boolean = false) => {
    ...
    try {
      ...
      const data = await webinarsFetchPromise;
      webinarsCache = { data, timestamp: Date.now(), loading: false, error: null };
      if (!isMountedRef.current) return;
      setWebinars(data);
      setLoading(false);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred while loading webinars.';
      webinarsCache = { data: [], timestamp: Date.now(), loading: false, error: errorMessage };
      if (!isMountedRef.current) return;
