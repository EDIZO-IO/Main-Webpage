...
const UpcomingWebinars: React.FC = () => {
  ...
  useEffect(() => {
    fetch("/webinars.json")
      .then((res) => res.json())
      .then((data) => {
        // ✅ Sort Confirmed first, then Coming Soon, both by date
        const sorted = data.sort((a: Webinar, b: Webinar) => {
          if (a.status === "Confirmed" && b.status !== "Confirmed") return -1;
          if (a.status !== "Confirmed" && b.status === "Confirmed") return 1;

          // Sort inside each group by date
    ...
    >
      ...
      <div className="w-full max-w-5xl relative z-10 text-center">
        ...
        {!loading && webinars.length === 0 && (
          <p className="text-slate-400">No webinars available right now. 🚀</p>
        )}
          ...
          {webinars.map((event) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                {/* ✅ Show date only if Confirmed */}
                {event.status === "Confirmed" && (
                  <time
                    dateTime={event.date}
                    className="text-slate-200 text-sm md:text-md font-medium group-hover:text-white transition-colors duration-300"
                  >
              ...
              {/* ✅ Show register button only if Confirmed */}
