  ...
  return (
    <div className="min-h-screen bg-white relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 opacity-[0.02]">
        {[Code, Palette, Video, Server, Bot, Film].map((Icon, i) => (
          <motion.div
            key={`bg-icon-${i}`}
          ...
          >
            <Icon size={50 + i * 8} strokeWidth={0.3} />
          </motion.div>
        ))}
      </div>

      {/* Hero Section - Video Background */}
      <section className="relative pt-28 pb-20 md:pt-36 md:pb-28 overflow-hidden min-h-[500px]">
        {/* Video Background */}
        <video
          autoPlay
          loop
        ...
        {/* Animated color orbs */}
        <motion.div
          className="absolute w-[500px] h-[500px] -top-48 -left-48 rounded-full z-[2]"
        ...
        />
        <motion.div
          className="absolute w-[400px] h-[400px] -bottom-32 -right-32 rounded-full z-[2]"
          style={{ background: 'radial-gradient(circle, rgba(6,182,212,0.25) 0%, transparent 70%)' }}
          animate={{ scale: [1.2, 1, 1.2], x: [0, -50, 0] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 3 }}
        />
        <motion.div
          className="absolute w-[300px] h-[300px] top-1/2 left-1/4 rounded-full z-[2]"
        ...
        <div className="absolute left-0 top-0 bottom-0 w-1/4 overflow-hidden pointer-events-none z-[3]">
          {[
            { Icon: Palette, color: '#f43f5e', size: 24 },
            { Icon: Code, color: '#22c55e', size: 20 },
            { Icon: Video, color: '#f97316', size: 22 },
            { Icon: Bot, color: '#06b6d4', size: 20 },
          ].map((item, i) => (
            <motion.div
              key={`left-${i}`}
              className="absolute"
        ...
        <div className="absolute right-0 top-0 bottom-0 w-1/4 overflow-hidden pointer-events-none z-[3]">
          {[
            { Icon: Server, color: '#8b5cf6', size: 22 },
            { Icon: PenTool, color: '#ec4899', size: 20 },
            { Icon: BarChart, color: '#f59e0b', size: 24 },
            { Icon: Film, color: '#fb923c', size: 20 },
          ].map((item, i) => (
            <motion.div
              key={`right-${i}`}
              className="absolute"
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="max-w-5xl mx-auto text-center">
            <AnimatedSection>
              <motion.div
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-full shadow-lg mb-8"
